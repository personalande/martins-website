...
import { buildQuoteMessage, getWhatsAppUrl } from '@/lib/whatsapp/message'
...
export async function GET(_req: NextRequest, { params }: Params) {
  try {
    ...
    const { data: quoteRaw, error } = await supabase
      .from('quotes')
      .select(`
        id, protocol, customer_name, customer_phone, store_preference,
        notes, public_total_estimate, created_at,
        quote_items (
    ...
    // Determine store WhatsApp
    const storeSlug = quoteRaw.store_preference
    const store = SITE_CONFIG.stores.find((s) => s.slug === storeSlug)
    const phone = store?.whatsapp || SITE_CONFIG.defaultWhatsapp
